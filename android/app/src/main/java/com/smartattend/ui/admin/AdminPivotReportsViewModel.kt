...
@HiltViewModel
class AdminPivotReportsViewModel @Inject constructor(
    private val repo: AdminRepository
) : ViewModel() {

    private val _reports = MutableStateFlow<Resource<PivotReportResponse>>(Resource.Loading)
    ...
    fun load(from: String? = null, to: String? = null, classId: String? = null, subjectId: String? = null) {
        viewModelScope.launch {
            _reports.emit(Resource.Loading)
            _reports.emit(repo.getPivotReport(from, to, classId, subjectId))
        }
    }
}
